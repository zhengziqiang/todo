...
template <typename Dtype>
void LSTMUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,//这里的vector代表的是个数组
  const vector<Blob<Dtype>*>& top) {
  input_height_ = bottom[0]->height();
  input_width_ = bottom[0]->width();
  output_height_ = (input_height_ - kernel_height_) / stride_height_ + 1;//计算公式
  output_width_ = (input_width_ - kernel_width_) / stride_width_ + 1;
...
void LSTMUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const Dtype* bottom_data = bottom[0]->cpu_data();//输入层数据不可变,输出层可变
